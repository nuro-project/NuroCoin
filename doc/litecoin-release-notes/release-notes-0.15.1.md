Nurocoin Core version *0.15.1* is now available from:

  <https://download.nurocoin.org/nurocoin-0.15.1/>
...
A number of changes to the way Nurocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 neurons per kB (0.00001 NURO). This is to prepare for dropping the minimum transaction fee to 0.1 neurons per kB (0.0001 NURO) in the next release.
